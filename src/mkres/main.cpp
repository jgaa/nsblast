...
int main(int argc, char *argv[]) {
    ...
    hdr << "#include <unordered_map>" << endl
        << "#include <string>" << endl << endl
        << "#include <string_view>" << endl << endl
        << "namespace " << ns << '{' << endl << endl
        << "using resource_t = std::unordered_map<std::string, std::string_view>;" << endl
    ...
    for(int i = 5; i < argc; i++) {
        const filesystem::path path = argv[i];
